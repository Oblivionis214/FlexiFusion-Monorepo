...
function addressToBytes32(address) {
  if (isSolanaAddress(address)) {
    return import_bs58.default.decode(address);
...
function parsePrecrimeConfigV2(precrimeConfig) {
  const buffer = import_buffer.Buffer.from(trim0x(precrimeConfig), "hex");
...
function optionsType1(_extraGas) {
  ...
  return pack(["uint16", "uint256"], [1, extraGas]);
}
function optionsType2(_extraGas, _dstNativeAmt, _dstNativeAddress) {
  ...
  return pack(
    ...
    [2, BigNumber.from(extraGas), BigNumber.from(dstNativeAmt), _dstNativeAddress]
  );
}
...
var ExecutorOptionType = ((ExecutorOptionType2) => {
...
var VerifierOptionType = ((VerifierOptionType2) => {
...
var Options = class _Options {
  ...
  decodeExecutorLzReceiveOption() {
    ...
    for (const option of options) {
      const buffer = import_buffer.Buffer.from(trim0x(option.params), "hex");
  ...
  decodeExecutorNativeDropOption() {
    ...
    const results = options.reduce((acc, cur) => {
      const buffer = import_buffer.Buffer.from(trim0x(cur.params), "hex");
  ...
  findVerifierOption(verifierIdx, optionType) {
    const worker = this.workerOptions.find(
      (w) => w.workerId === 2
      /* VERIFIER */
    );
    if (worker) {
...
var PacketV1Codec = class _PacketV1Codec {
  ...
  constructor(payloadEncoded) {
    ...
    this.buffer = import_buffer.Buffer.from(trim0x(payloadEncoded), "hex");
  }
  /**
  ...
  static fromBytes(payload) {
    return new _PacketV1Codec("0x" + import_buffer.Buffer.from(payload).toString("hex"));
  }
  /**
  ...
  static encode(packet) {
    ...
    return "0x" + import_buffer.Buffer.from(buff).toString("hex");
  }
  /**
  ...
  static encodeBytes(packet) {
    ...
    const buffer = import_buffer.Buffer.alloc(MESSAGE_OFFSET + message.length / 2);
    ...
    buffer.write(import_buffer.Buffer.from(addressToBytes32(packet.sender)).toString("hex"), SRC_ADDRESS_OFFSET, 32, "hex");
    buffer.writeUInt32BE(packet.dstEid, DST_CHAIN_OFFSET);
    buffer.write(import_buffer.Buffer.from(addressToBytes32(packet.receiver)).toString("hex"), DST_ADDRESS_OFFSET, 32, "hex");
...
var TimestampBlockConfiguration = ((TimestampBlockConfiguration2) => {
...
var Header = (_a = class extends Codec {
  ...
  constructor(globalVersion, appCommandLabel, requestCount) {
    super();
    __publicField(this, "globalVersion");
    // 2 bytes
    __publicField(this, "appCommandLabel");
    // 2 bytes
    __publicField(this, "requestCount");
    this.globalVersion = globalVersion;
    this.appCommandLabel = appCommandLabel;
    this.requestCount = requestCount;
    this.byteLength = _a.GLOBAL_VERSION_BYTES + _a.APP_COMMAND_LABEL_BYTES + _a.REQUEST_COUNT_BYTES;
  }
  /**
   ...
   */
  encode() {
    const globalVersionHex = numberToHex(this.globalVersion, _a.GLOBAL_VERSION_BYTES);
    const appCommandLabelHex = this.appCommandLabel.padStart(bytesToHexLength(_a.APP_COMMAND_LABEL_BYTES), "0");
    const requestCountHex = numberToHex(this.requestCount, _a.REQUEST_COUNT_BYTES);
  ...
  static decode(hex, offset) {
    ...
    const globalVersion = hexToNumber(sliceHex(hex, internalOffset, _a.GLOBAL_VERSION_BYTES));
    internalOffset += bytesToHexLength(_a.GLOBAL_VERSION_BYTES);
    const appCommandLabel = sliceHex(hex, internalOffset, _a.APP_COMMAND_LABEL_BYTES);
    internalOffset += bytesToHexLength(_a.APP_COMMAND_LABEL_BYTES);
    const requestCount = hexToNumber(sliceHex(hex, internalOffset, _a.REQUEST_COUNT_BYTES));
    return new _a(globalVersion, appCommandLabel, requestCount);
  }
}, // 2 bytes
__publicField(_a, "GLOBAL_VERSION_BYTES", 2), __publicField(_a, "APP_COMMAND_LABEL_BYTES", 2), __publicField(_a, "REQUEST_COUNT_BYTES", 2), _a);
var _a2;
var RequestHeader = (_a2 = class extends Codec {
  /**
  ...
  constructor(requestVersion, appRequestLabel, resolverType, requestSize) {
    super();
    __publicField(this, "requestVersion");
    // 1 byte
    __publicField(this, "appRequestLabel");
    // 2 bytes (hex or string)
    __publicField(this, "resolverType");
    // 2 bytes
    __publicField(this, "requestSize");
    this.requestVersion = requestVersion;
    this.appRequestLabel = appRequestLabel;
    this.resolverType = resolverType;
    this.requestSize = requestSize;
    this.byteLength = _a2.REQUEST_VERSION_BYTES + _a2.APP_REQUEST_LABEL_BYTES + _a2.RESOLVER_TYPE_BYTES + _a2.REQUEST_SIZE_BYTES;
  }
  /**
   ...
   */
  encode() {
    const requestVersionHex = numberToHex(this.requestVersion, _a2.REQUEST_VERSION_BYTES);
    const appRequestLabelHex = this.appRequestLabel.padStart(
      bytesToHexLength(_a2.APP_REQUEST_LABEL_BYTES),
      "0"
    );
    const resolverTypeHex = numberToHex(this.resolverType, _a2.RESOLVER_TYPE_BYTES);
    const requestSizeHex = numberToHex(this.requestSize, _a2.REQUEST_SIZE_BYTES);
  ...
  static decode(hex, offset) {
    let internalOffset = offset ?? 0;
    const requestVersion = hexToNumber(sliceHex(hex, internalOffset, _a2.REQUEST_VERSION_BYTES));
    internalOffset += bytesToHexLength(_a2.REQUEST_VERSION_BYTES);
    const appRequestLabel = sliceHex(hex, internalOffset, _a2.APP_REQUEST_LABEL_BYTES);
    internalOffset += bytesToHexLength(_a2.APP_REQUEST_LABEL_BYTES);
    const resolverType = hexToNumber(sliceHex(hex, internalOffset, _a2.RESOLVER_TYPE_BYTES));
    internalOffset += bytesToHexLength(_a2.RESOLVER_TYPE_BYTES);
    const requestSize = hexToNumber(sliceHex(hex, internalOffset, _a2.REQUEST_SIZE_BYTES));
    return new _a2(requestVersion, appRequestLabel, resolverType, requestSize);
  }
}, // 2 bytes
__publicField(_a2, "REQUEST_VERSION_BYTES", 1), __publicField(_a2, "APP_REQUEST_LABEL_BYTES", 2), __publicField(_a2, "RESOLVER_TYPE_BYTES", 2), __publicField(_a2, "REQUEST_SIZE_BYTES", 2), _a2);
var _a3;
var EVMBase = (_a3 = class extends Codec {
  /**
  ...
  constructor(targetEid, timestampBlockFlag, blockConfirmations, to, timestamp, blockNumber) {
    super();
    __publicField(this, "targetEid");
    // 4 bytes
    __publicField(this, "timestampBlockFlag");
    // 1 byte
    __publicField(this, "blockConfirmations");
    // 2 bytes
    ...
    this.byteLength = _a3.TARGET_EID_BYTES + _a3.TIMESTAMP_BLOCK_FLAG_BYTES + _a3.BLOCK_CONFIRMATIONS_BYTES + _a3.TO_BYTES + (timestamp !== void 0 ? _a3.TIMESTAMP_BYTES : 0) + (blockNumber !== void 0 ? _a3.BLOCK_NUMBER_BYTES : 0);
  }
  /**
   ...
   */
  encode() {
    const targetEidHex = numberToHex(this.targetEid, _a3.TARGET_EID_BYTES);
    const timestampBlockFlagHex = numberToHex(this.timestampBlockFlag, _a3.TIMESTAMP_BLOCK_FLAG_BYTES);
    const blockNumberHex = this.blockNumber !== void 0 ? bigIntToHex(this.blockNumber, _a3.BLOCK_NUMBER_BYTES) : void 0;
    const timestampHex = this.timestamp !== void 0 ? bigIntToHex(this.timestamp, _a3.TIMESTAMP_BYTES) : void 0;
    if (timestampHex !== void 0 && blockNumberHex !== void 0) {
      throw new Error("Cannot have both timestamp and block number");
    }
    const blockConfirmationsHex = numberToHex(this.blockConfirmations, _a3.BLOCK_CONFIRMATIONS_BYTES);
    const toHex = this.to.padStart(bytesToHexLength(_a3.TO_BYTES), "0");
  ...
  static decode(hex, offset) {
    let internalOffset = offset ?? 0;
    const targetEid = hexToNumber(sliceHex(hex, internalOffset, _a3.TARGET_EID_BYTES));
    internalOffset += bytesToHexLength(_a3.TARGET_EID_BYTES);
    const timestampBlockFlag = hexToNumber(
      sliceHex(hex, internalOffset, _a3.TIMESTAMP_BLOCK_FLAG_BYTES)
    );
    internalOffset += bytesToHexLength(_a3.TIMESTAMP_BLOCK_FLAG_BYTES);
    let timestamp;
    let blockNumber;
    if (timestampBlockFlag === 0) {
      timestamp = hexToBigInt(sliceHex(hex, internalOffset, _a3.TIMESTAMP_BYTES));
      internalOffset += bytesToHexLength(_a3.TIMESTAMP_BYTES);
    } else {
      blockNumber = hexToBigInt(sliceHex(hex, internalOffset, _a3.BLOCK_NUMBER_BYTES));
      internalOffset += bytesToHexLength(_a3.BLOCK_NUMBER_BYTES);
    }
    const blockConfirmations = hexToNumber(sliceHex(hex, internalOffset, _a3.BLOCK_CONFIRMATIONS_BYTES));
    internalOffset += bytesToHexLength(_a3.BLOCK_CONFIRMATIONS_BYTES);
    const to = sliceHex(hex, internalOffset, _a3.TO_BYTES);
    return new _a3(targetEid, timestampBlockFlag, blockConfirmations, to, timestamp, blockNumber);
  }
}, // 8 bytes (if timestampBlockFlag indicates block number)
__publicField(_a3, "TARGET_EID_BYTES", 4), __publicField(_a3, "TIMESTAMP_BLOCK_FLAG_BYTES", 1), __publicField(_a3, "BLOCK_CONFIRMATIONS_BYTES", 2), __publicField(_a3, "TO_BYTES", 20), __publicField(_a3, "TIMESTAMP_BYTES", 8), __publicField(_a3, "BLOCK_NUMBER_BYTES", 8), _a3);
var SingleViewFunctionEVMCall = class _SingleViewFunctionEVMCall extends EVMBase {
  ...
  constructor(requestHeader, targetEid, timestampBlockFlag, blockConfirmations, to, calldata, timestamp, blockNumber) {
    ...
    __publicField(this, "requestHeader");
...
var _a4;
var ComputeHeader = (_a4 = class extends Codec {
  /**
  ...
  constructor(computeVersion, computeType, computeSetting) {
    super();
    __publicField(this, "computeVersion");
    // 1 byte
    __publicField(this, "computeType");
    // 2 bytes
    __publicField(this, "computeSetting");
    this.computeVersion = computeVersion;
    this.computeType = computeType;
    this.computeSetting = computeSetting;
    this.byteLength = _a4.COMPUTE_VERSION_BYTES + _a4.COMPUTE_TYPE_BYTES + _a4.COMPUTE_SETTING_BYTES;
  }
  /**
   ...
   */
  encode() {
    const computeVersionHex = numberToHex(this.computeVersion, _a4.COMPUTE_VERSION_BYTES);
    const computeTypeHex = numberToHex(this.computeType, _a4.COMPUTE_TYPE_BYTES);
    const computeSettingHex = numberToHex(this.computeSetting, _a4.COMPUTE_SETTING_BYTES);
  ...
  static decode(hex, offset) {
    let internalOffset = offset ?? 0;
    const computeVersion = hexToNumber(sliceHex(hex, internalOffset, _a4.COMPUTE_VERSION_BYTES));
    internalOffset += bytesToHexLength(_a4.COMPUTE_VERSION_BYTES);
    const computeType = hexToNumber(sliceHex(hex, internalOffset, _a4.COMPUTE_TYPE_BYTES));
    internalOffset += bytesToHexLength(_a4.COMPUTE_TYPE_BYTES);
    const computeSetting = hexToNumber(sliceHex(hex, internalOffset, _a4.COMPUTE_SETTING_BYTES));
    return new _a4(computeVersion, computeType, computeSetting);
  }
}, // 1 byte
__publicField(_a4, "COMPUTE_VERSION_BYTES", 1), __publicField(_a4, "COMPUTE_TYPE_BYTES", 2), __publicField(_a4, "COMPUTE_SETTING_BYTES", 1), _a4);
var ComputeEVM = class _ComputeEVM extends EVMBase {
  ...
  constructor(computeHeader, targetEid, timestampBlockFlag, blockConfirmations, to, timestamp, blockNumber) {
    super(targetEid, timestampBlockFlag, blockConfirmations, to, timestamp, blockNumber);
    __publicField(this, "computeHeader");
...
export {
  ...
  optionsType1,
  optionsType2,
  packetToMessageOrigin,
  parseError,
  parsePrecrimeConfig,
  trim0x
};
//# sourceMappingURL=@layerzerolabs_lz-v2-utilities.js.map
